...
itemPriceElement.textContent = getTaxText(totalPrice);
    ...
    document.getElementById('add-to-cart').addEventListener('click', () => {
      ...
      document.querySelectorAll('.option-item.selected').forEach(optionDiv => {
        ...
        const optionName = optionDiv.querySelector('.option-name').textContent; // ← これを追加
        const additionalPrice = parseFloat(optionDiv.getAttribute('data-price'));
        selectedOptions.push({
          id: optionId,
          name: optionName,                 // ← name を保存
  ...
  if (orderList.order[selectedName]) {
    totalQuantity = orderList.order[selectedName]
    ...
    orderList.order[selectedName].forEach((item, index) => {
      ...
      if (item.options && item.options.length > 0) {
        item.options.forEach(option => {
          ...
          optionLi.textContent = `(${option.name} +￥${option.additional_price})`;
          optionLi.style.marginLeft = '20px';
          cartItemsContainer.appendChild(optionLi);
        });
      }
