...
function resetQuantityInput() {
    ...
    quantityButtons.forEach(button => {
        button.classList.toggle('active', button.getAttribute('data-value') === '1');
    });
}
...
quantityButtons.forEach(button => {
    button.onclick = () => {
        ...
        quantityInput.value = button.getAttribute('data-value');
    };
});
...
confirmQuantityBtn.onclick = handleConfirmQuantity;
...
document.getElementById('save-add-menu').addEventListener('click', async () => {
 ...
 try {
   ...
   if (response.ok) {
     ...
     if (orderIndex !== -1) {
         pendingOrders[orderIndex] = responseData;
     } else {
         pendingOrders.push(responseData);
     }
     ...
     const refreshedOrder = clients.pendingOrders.find(order => order.id === responseData.id) || responseData;
     const refreshedCard = ordersList.querySelector(`[data-id="${responseData.id}"]`);
     if (refreshedCard) {
       refreshedCard.classList.add('selected-card');
       selectedCard = refreshedCard;
     }

     displayOrderDetails(refreshedOrder, {
       MainData,
       clients,
       currentLang,
       orderItems,
